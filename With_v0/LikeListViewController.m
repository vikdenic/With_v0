...
- (void)usersAttendingQuery
{
    ...
    {
        PFUser *user = [likeActivity objectForKey:@"fromUser"];
        [thisArray addObject:user];
    }

    for (PFUser *user in thisArray)
    {
        ...
        [query getObjectInBackgroundWithId:userId block:^(PFObject *object, NSError *error)
         {
             ...
             [self.usersAttendingArray addObject:object];
...
{
    LikesListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell"];
...
- (void)ontapped:(LikesListFriendButton *)sender
{
...
- (void)onButtonTitlePressed:(UIButton *)sender
{
    ...
    [self performSegueWithIdentifier:@"LikesListToStreamProfile" sender:self];
}

-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"LikesListToStreamProfile"])
    {
